...
class GAMSImporter(JSONPlugin):
	...
	try:
        ...
        if type(filename) is list:
            ...
            self.is_MGS=True
            self.get_mga_index(str(filename[0]))
            self.filename=str(filename[1])
        else:
            self.is_MGS = False
        ...
        for link in self.network.links:
            ...
            for attr in link.attributes:
                if attr.attr_is_var == 'Y':
                        ...
                        if self.attrs[attr.attr_id] in self.gdx_variables.keys():
                            ...
                            if gdxvar.name in self.gdx_ts_vars.keys():
                                ...
                                if gdxvar.dim == 4:
                                    ...
                                    for i, idx in enumerate(gdxvar.index):
                                        if idx[0] == self.MGA_index[j] and '_'.join(idx[1:]).lower() == link.name.lower():
                                            data = gdxvar.data[i]
                                            try:
                                                data_ = float(data)
                                                dataset['type'] = 'scalar'
	                ...
	                #if self.attrs[attr.attr_id].lower().startswith('capex_weff_met') and link.name.lower().startswith('bourne-wsm2'):
